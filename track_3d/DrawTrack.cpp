...
void CTrackDraw::on_mouse_move(const CPoint& point)
{
  ...
  {
    CRay ray = get_view_dir(point);
    pReg = intersect(ray, m_FaceUnderCursor);
  ...
  std::string cRgnName = pReg != NULL ? "Region = " + pReg->sName : " ";
  ...
  if(pStatusBar)
  {
    pStatusBar->SetPaneText(1, cRgnName.c_str());
    pStatusBar->SetPaneText(2, cTextX.c_str());
    pStatusBar->SetPaneText(3, cTextY.c_str());
    pStatusBar->SetPaneText(4, cTextZ.c_str());
  }
}
